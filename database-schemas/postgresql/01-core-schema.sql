-- PostgreSQL Core Schema for Payments Engine v2
-- Enhanced with UETR correlation and ISO 20022 support

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create database if not exists
-- CREATE DATABASE payments_engine_v2;
...
-- Payment audit log for immutable audit trails
...
COMMENT ON TABLE payments IS 'Core payment data with UETR correlation for ISO 20022 compliance';
COMMENT ON TABLE uetr_correlation IS 'UETR-based message correlation for ISO 20022 messages';
COMMENT ON TABLE iso20022_message_details IS 'ISO 20022 message details with correlation information';
COMMENT ON TABLE payment_audit_log IS 'Immutable audit trail for payment operations';
COMMENT ON TABLE payment_status_history IS 'Payment status change history for audit and compliance';
COMMENT ON TABLE tenant_configuration IS 'Tenant-specific configuration settings';

COMMENT ON COLUMN payments.uetr IS 'ISO 20022 Unique End-to-End Transaction Reference';
COMMENT ON COLUMN uetr_correlation.uetr IS 'ISO 20022 UETR for message correlation';
COMMENT ON COLUMN iso20022_message_details.uetr IS 'UETR reference for message correlation';
COMMENT ON COLUMN payment_audit_log.hash IS 'SHA-256 hash for integrity verification';
